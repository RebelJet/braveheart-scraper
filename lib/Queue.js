...
class Queue {
  ...
  async setupVpn(vpnAccount) {
    const vpn = new OpenVPN(vpnAccount, err => this.restart());
    await vpn.setup();
    this.addRoute();
  ...
  announceReady() {
    const resp = encodeMsgToDispatcher(110, '', 'READY');
    console.log(`ANNOUNCE WE'RE READY FOR JOBS!!`)
    this.send(resp);
  }
  ...
  restart() {
    console.error('restart() called!');
    this.isAborting = true;
    this.destroyClient();
    spawnSync('/app/scripts/killAndExit.sh');
  }

  async refresh() {
    console.error('refresh() called!');

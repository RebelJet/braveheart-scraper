  ...
  return function Plugin(req) {
    ...
    self.run = async function run() {
      ...
      if (isDebugMode && recentFileExists(`${filepath}/results.json`)) {
        ...
        req.data = await runFetchDataFromSite(req, browser, fetchDataFromSite);
...
function runFetchDataFromSite(req, browser, fetchDataFromSite) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const data = await fetchDataFromSite(req, browser);
      resolve(data);
    } catch(err) {
      err = (onError ? onError(err): null) || err;
      reject(err)
